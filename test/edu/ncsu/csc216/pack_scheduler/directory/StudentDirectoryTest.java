...
public class StudentDirectoryTest {
	...
	public void testLoadStudentsFromFile() {
		...
		try {
			sd.newStudentDirectory();
			sd.loadStudentsFromFile("not_a_file.txt");
			fail();
	...
	public void testAddStudent() {
		...
		//Test if the passwords are invalid
		try {
		...
		try {
			sd.addStudent(FIRST_NAME, LAST_NAME, ID, EMAIL, PASSWORD, "notpassword", MAX_CREDITS);
			fail();
		} catch (IllegalArgumentException e) {
			//Passed
		}
	...
	/**
	 * Tests getting student when passed in the ID
	 */
	@Test
	public void testGetStudentById() {
		StudentDirectory sd = new StudentDirectory();
		sd.addStudent(FIRST_NAME, LAST_NAME, ID, EMAIL, PASSWORD, PASSWORD, MAX_CREDITS);
		Student s = new Student(FIRST_NAME, LAST_NAME, ID, EMAIL, "0ÉRú\"ÃùuŸ¦Ù\\7X²F´þâ9•{-OîFâapÄ", MAX_CREDITS);
		assertEquals(s.getId(), sd.getStudentById(ID).getId());
		assertNull(sd.getStudentById("thisisanid"));
	}
